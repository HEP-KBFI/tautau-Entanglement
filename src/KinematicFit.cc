...
KinematicFit::operator()(const KinematicEvent& kineEvt)
{
  ...
  int numSignCombinations = -1;
  if      ( applyTauMassConstraint_ == 0 ) numSignCombinations = 1;
  else if ( applyTauMassConstraint_ == 1 ) numSignCombinations = 4;
  else throw cmsException("KinematicFit::operator()", __LINE__) 
         << "Invalid Configuration parameter 'applyTauMassConstraint' !!";

  KinematicEvent kineEvt_kinfit = kineEvt;

  double min_chi2 = -1.;
  int status = -1;
  for ( int idxSignCombination = 0; idxSignCombination < numSignCombinations; ++idxSignCombination )
  {
    int tauPlusSign  = ( (idxSignCombination % 2) == 0 ) ? kPlusSign : kMinusSign;
    int tauMinusSign = ( (idxSignCombination / 2) == 0 ) ? kPlusSign : kMinusSign;
    if ( verbosity_ >= 1 )
    {
      std::cout << "processing SignCombination #" << idxSignCombination << ":" 
                << " sign(tau+) = " << tauPlusSign << ", sign(tau-) = " << tauMinusSign << "\n";
    }
    ...
    while ( !hasConverged && iteration <= max_iterations )
    {
      ...
      else if ( applyTauMassConstraint_ == 1 )
      {
        ...
        double signFactor = 0.;
        if      ( tauPlusSign == kPlusSign  ) signFactor = +1.;
        else if ( tauPlusSign == kMinusSign ) signFactor = -1.;
        D( 1, 3) = (-2.*signFactor*square(mTau)*visTauPlusE*tauPlusPx - square(visTauPlusE)*tauPlusE*tauPlusPx
        ...
        D( 1, 4) = (-2.*signFactor*square(mTau)*visTauPlusE*tauPlusPy - square(visTauPlusE)*tauPlusE*tauPlusPy
                   + tauPlusE*(-square(mTau)*tauPlusPy
                              ...
                              + visTauPlusPy*(square(visTauPlusP) + 2.*square(nuTauPlusPx) + 2.*nuTauPlusPx*visTauPlusPx
                                                                  + 2.*square(nuTauPlusPz) + 2.*nuTauPlusPz*visTauPlusPz)))/denominator1;
        D( 1, 5) = (-2.*signFactor*square(mTau)*visTauPlusE*tauPlusPz - square(visTauPlusE)*tauPlusE*tauPlusPz
        ...
        d( 1) = comp_cosThetaGJ(tauPlusP4, visTauPlusP4) - comp_cosThetaGJ_solution(tauPlusP4, visTauPlusP4, tauPlusSign);
      }
      else assert(0);
      ...
      else if ( applyTauMassConstraint_ == 1 )
      {
        ...
        double signFactor = 0.;
        if      ( tauMinusSign == kPlusSign  ) signFactor = +1.;
        else if ( tauMinusSign == kMinusSign ) signFactor = -1.;
        D( 4, 9) = (-2.*signFactor*square(mTau)*visTauMinusE*tauMinusPx - square(visTauMinusE)*tauMinusE*tauMinusPx
        ...
        D( 4,10) = (-2.*signFactor*square(mTau)*visTauMinusE*tauMinusPy - square(visTauMinusE)*tauMinusE*tauMinusPy
                   + tauMinusE*(-square(mTau)*tauMinusPy
                               ...
                               + visTauMinusPy*(square(visTauMinusP) + 2.*square(nuTauMinusPx) + 2.*nuTauMinusPx*visTauMinusPx
                                                                     + 2.*square(nuTauMinusPz) + 2.*nuTauMinusPz*visTauMinusPz)))/denominator1;
        D( 4,11) = (-2.*signFactor*square(mTau)*visTauMinusE*tauMinusPz - square(visTauMinusE)*tauMinusE*tauMinusPz
        ...
        d( 4) = comp_cosThetaGJ(tauMinusP4, visTauMinusP4) - comp_cosThetaGJ_solution(tauMinusP4, visTauMinusP4, tauMinusSign);
      }
      else assert(0);
      // CV: add "parallelism" constraint for tau-
      //     The constraint equations H_Pphi and H_Ptheta are given by Eqs. (4.10) and (4.11)
      ...
      d(12) = visTauPlusE  + nuTauPlusE  + visTauMinusE  + nuTauMinusE  - recoilPx;
      if ( verbosity_ >= 1 )
      {
        std::cout << "D:\n";
        std::cout << D << "\n";
